...
| **petejohanson/cirque-input-module (our upstream)** | Relative-only: `input_report_rel` only, no `input_report_abs`, no Z | — | — | "We added absolute mode upstream" is a **legitimately novel claim in ZMK** |
...
| **UHK touchpad module** | Azoteq IQS572 does gestures **on-chip**; firmware receives pre-quantized `int8` wheel ticks | Multitouch keyboard trackpad | Absolute per-finger data never leaves the sensor IC | Nice illustration of why the Pinnacle's raw access is the enabling hardware property |
