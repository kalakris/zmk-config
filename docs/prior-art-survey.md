...
**State the gap narrowly (added 2026-09-05).** Do *not* claim that
trackpad-quality scroll is unreachable without a vendor touch protocol — that
is false and a reviewer will say so. The accurate, defensible claim is one
sentence: **a motion-gated relative wheel stream cannot distinguish a finger
resting motionless on the pad from a finger that has lifted, and every felt
difference follows from that one missing bit.** Phantom momentum when a finger
comes to rest, and the inability to halt a coast by planting a finger, are both
that bit. Everything else in the comparison is a matter of degree, and some of
it is recoverable host-side (§6.12). What raw touch buys is not "the only way"
but a *measured* rather than inferred touch lifecycle, plus absolute positions
that leave an extension path to multi-finger the wheel path forecloses.
...
12. **"You didn't need any of this — stock LinearMouse plus a declared scroll resolution gets 90% of it."** ⚠️ **Added 2026-09-05. The strongest good-faith objection on this list, and partly correct.** Upstream's `SmoothedScrollingEngine` already supplies phase and momentum (item 3); [zmk#3483](https://github.com/zmkfirmware/zmk/pull/3483) removes macOS's 9-counts-per-inch quantization. That pairing is a real no-custom-host alternative for the common case — flick, scroll, coast, catch — not a coarse fallback. Do not overstate the gap. Answer honestly on three points:

    - **What survives.** The finger-down bit (§3): a resting finger reads as a lift, so momentum fires spuriously and cannot be halted by planting a finger. That is felt, not theoretical.
    - **What does *not* survive, and concede it.** Pad identity and device-time reconstruction are both recoverable without a touch protocol. LinearMouse's `DeviceManager` already runs an IOHIDManager and attributes tapped CGEvents to whichever device fired last (`lastActiveDeviceId`) — extending that to per-pad report IDs is an extension of existing code; and cross-pad arbitration is better solved in *firmware*, which already knows both pads, than on the host at all. BLE batching is heuristically undoable at a known fixed 100 SPS. The blocker there is only that reports are motion-gated (our own `patches/zmk-skip-empty-mouse-report-syncs.patch`), which a touch-gated heartbeat would fix — and that same heartbeat would also supply the finger-down bit. **So the honest floor is not "you need raw touch"; it is "you need *some* side channel," and ours carries more bits than the minimum.**
    - **Why more bits anyway.** Absolute positions keep a multi-finger extension path open (docs/tps43-bench.md) that a 1-bit flag forecloses; a heartbeat costs a BLE notification per sample regardless of motion; and the minimal design requires re-forking LinearMouse with a change too device-specific to upstream — the fork we just deliberately retired. These are trade-offs, not refutations. Present them as such.

    Cheap evidence to have in hand before publishing: quit RawTouch → Standard mode, enable LinearMouse smoothed scrolling, A/B on `bench/safari-bounce/`. Not yet run.
