...
- `~/src/linearmouse` (`kalakris/linearmouse`) — host consumer. Since 2026-08-28: **`main` = upstream + the touch-stream feature** (review cleanups re-landed — the old revert saga is closed, batch exonerated); **`inputscale` = main + the 2 generic `inputScale` commits** (upstream-PR candidate, kept rebased on main) — deploys come from `inputscale`. Fork CI runs the unsigned workflow (signed pipeline is gated upstream-only). Old `go60-inputscale` branch deleted; local tag `pre-restructure-go60-inputscale` keeps its history
