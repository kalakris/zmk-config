...
arbitration. Dual-mode: standard pointer + Nav-layer ÷24 wheel events
remain as a driverless fallback; the host suppresses the wheel events per
physical device identity (the Sofle shares ZMK's default VID/PID).
...
- `~/src/linearmouse` (`kalakris/linearmouse`) — host consumer. Restructured 2026-08-28: **`main` = upstream + the touch-stream feature** (32 commits); **`inputscale` = main + the 2 generic `inputScale` commits** (upstream-PR candidate, kept rebased on main) — deploys come from `inputscale`. Old `go60-inputscale` branch kept only so the re-land plan's SHAs stay reachable; delete after item (b). Note: the review-pass cleanup batch `a204c40` is reverted (`29a8f88`, live regression); re-land item-by-item per docs/linearmouse-reland-plan.md
